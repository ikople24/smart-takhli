...
- **Officer tasks / งานเจ้าหน้าที่** — โมดูล `tasks` (เฟส 1–2, 2026-09-01 — ดีไซน์ `docs/design_handoff_officer_task_management/`; หน้า `/admin/my-tasks` โฉมใหม่ใช้งานได้แล้ว, `task-pool`/หน้าจอ 3 ยังไม่มี): logic ล้วนมีเทสต์ใน `lib/tasks/` (SLA/derived/KPI/groupBy/badges — **API เป็นคนคำนวณ derived fields (`deriveAssignment`) client ห้ามคำนวณซ้ำ**), API `pages/api/tasks/{my-kpi,pending,settings}.ts` (+`_auth.ts`), components `components/tasks/`, SLA ต่อประเภทเรื่องใน `models/tasks/TaskSettings.js` (collection `task_settings`, ไม่มีเอกสาร = default 7 วัน) — **เลิก hardcode 7 วัน**; `models/Assignment.js` ขยาย (role/stage/dueDate/slaPausedAt/coordination/blocked/timeline — เขียน coordination/โอนงานผ่าน `pages/api/complaints/{coordination,assignments/transfer}.ts` เท่านั้น); `Complaint.organizationId` ต้องมีทั้ง `models/Complaint.js` และ `models/SubmittedReport.js`; สถานะใหม่ `รอประสานหน่วยงานภายนอก` เป็นค่าคงที่ใน `lib/tasks/status.js` (ไม่ใส่ Mongoose enum); นับวันตามปฏิทินไทย (`lib/tasks/format.js`); สีใช้โทเคน `tk-*` ใน `styles/globals.css` **ไม่แตะ `--color-primary`**; รายละเอียด `docs/modules/tasks.md`
