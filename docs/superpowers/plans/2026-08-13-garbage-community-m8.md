...
**Architecture:** จับคู่ชื่อจุด → ถนนใน `roads` → ยิง `$geoIntersects` กับ polygon ใน `geojsonfeatures` → ได้ชื่อชุมชน · ทำได้ 77/172 รายการจุด (= 62 จากชื่อจุดไม่ซ้ำ 146 ชื่อ) ที่เหลือเป็นโรงเรียน/หมู่บ้าน/ตลาดซึ่งไม่ใช่ถนน ต้องให้เจ้าหน้าที่เลือกจาก dropdown · แยก `communitySource` เป็น auto/manual เพื่อไม่ให้สคริปต์ทับงานที่คนยืนยันแล้ว
...
Expected: **`เติมชุมชนได้ 77 จุด` จาก 172 รายการจุด** (= 62 จากชื่อจุดไม่ซ้ำ 146 ชื่อ — สคริปต์นับ**รายการจุดในทุกสาย** จุดชื่อเดียวกันที่อยู่หลายสายจึงนับหลายครั้ง) และรายการจุดที่เติมไม่ได้เป็นพวกโรงเรียน/หมู่บ้าน/ตลาด · **ถ้าตัวเลขต่างจาก 77 มากให้หยุดและรายงาน**
...
- audit มี `garbage_communities_mapped` **อย่างน้อย 1 รายการต่อการรันหนึ่งครั้ง** (รัน `--yes` สองครั้งตาม Step 3 จะได้ 2 รายการ — ถูกต้องแล้ว)
...
- `scripts/map-garbage-communities.mjs` เติมอัตโนมัติจากพิกัด (77/172 จุด = 62 จากชื่อจุดไม่ซ้ำ 146 ชื่อ) · **ไม่ทับค่าที่เป็น `manual`** · ที่เหลือเป็นโรงเรียน/หมู่บ้าน/ตลาดซึ่งไม่ใช่ถนน ต้องให้เจ้าหน้าที่เลือกจาก dropdown ที่หน้าจัดการสาย
...
- [ ] `tsc` / `lint` / `build` ผ่าน
- [ ] สคริปต์ dry-run ได้ 77 จุด จาก 172 รายการจุด (= 62 จากชื่อจุดไม่ซ้ำ 146 ชื่อ) · รันจริงแล้วรันซ้ำได้ผลเดิม
...
1. เข้า `/admin/garbage` → ปุ่มจัดการสาย → ไล่เลือกชุมชนให้จุดที่ยังว่าง (~95 จุด) และตรวจจุดที่ขึ้นพื้นเหลือง (ระบบเติมให้ ยังไม่ได้ยืนยัน)
