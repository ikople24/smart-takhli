...
## ผลการตัดสินใจระหว่างทำ M7 (แก้แผนเดิม)

dry-run รอบแรกทำตามโค้ดในแผนแล้วได้ **60 งาน** (ไม่ใช่ 56) และผลลัพธ์ขัดกฎของโมดูลเอง 2 ข้อ
ตรวจแล้วสรุปว่าเป็น**กฎที่ตั้งไว้ผิด ไม่ใช่ข้อมูลผิด** จึงแก้ตามนี้ (เจ้าของงานตัดสิน 2026-08-13):

1. **ถอดกฎ "เวลาใน stopTimes ต้องไม่ย้อนกลับตามลำดับ seq"** ออกจาก `assignmentSchema`
   — `seq` คือเลขแถวในรายชื่อสถานที่ ไม่ใช่ลำดับที่รถวิ่ง และรถวิ่งคนละเส้นทางในแต่ละวัน
   (`live.ts` เรียงจุดด้วย `atMin` อยู่แล้ว) · ถอดการคำนวณ `warnings` ที่อ้างกฎนี้ใน
   `routes/[code].ts` ด้วย แต่**คงคีย์ `warnings: []` ไว้** เพราะ `RouteManagerModal` อ่านอยู่
2. **หนึ่งรถ หนึ่งวัน = หนึ่งงาน** — รถออกทริปเดียว เก็บจุดตัวเองสลับกับจุดที่เก็บแทน
   การแยกสองงานทำให้ช่วง min–max คาบเกี่ยวกันเองแล้วชนกฎห้ามเวลาทับ · `stopTimes` รวมทุกจุด,
   `coverForRouteCode` = สายที่ไปเก็บแทน (ตั้งกับ `normal` ได้), `kind` = `substitute`
   เฉพาะวันที่ไม่มีจุดของตัวเองเลย, `label` = "เก็บแทนเบอร์ X"
   → ต้องผ่อนกฎ `coverForRouteCode` ใน `assignmentSchema` จาก "ใช้ได้เฉพาะ substitute"
   เป็น "ถ้ามี `coverForRouteCode` ต้องมี `routeCode`"
   → ได้ **56 งาน** ตรงกับที่แผนคาดไว้ และ overlap หายไปเอง
...
- รถ 5 วันอังคาร: มี**งานเดียว** ที่ `coverForRouteCode` เป็น `R1` (หนึ่งรถหนึ่งวันหนึ่งงาน — ดู "ผลการตัดสินใจ" ข้างล่าง)
- รถ 6 วันอังคาร: `kind` เป็น `substitute` (วันนั้นเก็บแทนเบอร์ 2 ล้วน ไม่มีจุดของตัวเอง)
