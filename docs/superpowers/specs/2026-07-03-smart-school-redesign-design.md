...
| ประเด็น | ตัดสินใจ |
|---|---|
| จับคู่รายเก่า (ข้อมูลเก่าไม่มีเลข 13 หลัก) | เลข 13 หลักก่อน → ไม่เจอให้ค้นชื่อเป็น fallback → ยืนยันตัวตน → ผูกเลข 13 หลักให้ record เก่าทันที |
| ผู้ใช้ฟอร์ม | ผสม (เจ้าหน้าที่ลงพื้นที่ + ประชาชนกรอกเอง) → ออกแบบยึด public เป็นหลัก: มาสก์ข้อมูล + ยืนยันตัวตนก่อนเห็นข้อมูลเดิม |
| โครงข้อมูล | แยก 2 ชั้น: ทะเบียนบุคคล + ใบสมัครรายปีงบประมาณ (ข้อมูลทุกปีอยู่ครบ) |
| สถานะงาน | เบา ๆ 3 ค่า: `รับคำร้อง → ตรวจสอบแล้ว \| ไม่ผ่านเกณฑ์` |
...
- `prefix`, `name`, `phone` — ค่าล่าสุดของบุคคล (phone ใช้ยืนยันตัวตน)
...
- ฟิลด์ข้อมูลเดิมครบ: `educationLevel`, `schoolName`, `gradeLevel`, `gpa`,
  ...
  `location: {lat, lng}`
- `status: String` enum `['รับคำร้อง', 'ตรวจสอบแล้ว', 'ไม่ผ่านเกณฑ์']`
...
- รายเก่า: กรอกเลข 13 หลัก (validate checksum ฝั่ง client ก่อนยิง) →
  ...
  - **เจอ** applicant ที่ `citizenId` ตรง → แสดงชื่อแบบมาสก์
    (เช่น "พงศกรณ์ ผ่xxx") → กรอกเบอร์โทรเดิม 10 หลัก →
    `POST /api/smart-school/verify` → ตรงจึงได้ข้อมูลใบสมัครปีล่าสุดมา prefill
  - **ไม่เจอ** → ค้นด้วยชื่อ-นามสกุล → รายการ match แบบมาสก์ (≤5 รายการ:
    ชื่อมาสก์ + ปีที่เคยยื่น เท่านั้น) → เลือกของตัวเอง → ยืนยันเบอร์โทรเดิม →
    verify สำเร็จ = **ผูก `citizenId` ให้ applicant นั้นทันที** + prefill
  - ยืนยันไม่ผ่าน → แจ้งชัดเจน + เสนอยื่นเป็นรายใหม่ (เจ้าหน้าที่เห็นรายการซ้ำ
...
กติกา PDPA: ก่อน verify สำเร็จ ห้ามคืน รายได้ / ที่อยู่ / เบอร์โทร / พิกัด /
รูป ใด ๆ ออกจาก API — คืนได้เฉพาะชื่อมาสก์ + ปีที่เคยยื่น

## 4) API — `pages/api/smart-school/`

| Endpoint | Auth | หน้าที่ |
|---|---|---|
| `POST lookup` | public | ค้นด้วย `citizenId` หรือ `name` → ผลมาสก์เท่านั้น ≤5 รายการ |
| `POST verify` | public | `applicantRef` (หรือ citizenId) + `phone` → ตรงเป๊ะจึงคืนข้อมูลเต็มปีล่าสุด; เส้นทางค้นชื่อ = ผูก citizenId ให้ด้วย |
...
- lookup/verify เป็น public: จำกัดผลลัพธ์, ไม่คืนข้อมูลอ่อนไหวก่อน verify,
  เทียบเบอร์โทรแบบตรงทั้ง 10 หลักเท่านั้น
...
- OTP/SMS ยืนยันตัวตน (ใช้เบอร์โทรเดิมพอ), การอนุมัติทุนหลายขั้น/คณะกรรมการ,
  RPA/LTAX, การรวม StudentFeedback เข้าโมดูลนี้ (คงแยกตาม roadmap activities),
