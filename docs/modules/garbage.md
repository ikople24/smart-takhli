...
| ชั้น | ไฟล์ |
|---|---|
| หน้าประชาชน | `pages/garbage.tsx` + `components/garbage/{GarbageHero,GarbageSearchPanel,DayOffNotice,RouteTimeline,PrepChecklist,CoverageNote,GarbageHomeCard,TruckSprite}.tsx` |
| รูปประกอบ | `public/garbage/truck-{yellow,green}.png` (200×200 · วงกลมใส่เลขคันอยู่ที่ 46% / 44%) |
...
- `scripts/map-garbage-communities.mjs` เติมอัตโนมัติจากพิกัด — เติมได้ 77/172 จุด (ที่เหลือเป็นโรงเรียน/หมู่บ้าน/ตลาดซึ่งไม่ใช่ถนน) · **ไม่ทับค่าที่เป็น `manual`**
- **สถานะปัจจุบัน (ตรวจ 2026-08-22): ครบ 172/172 จุด และเป็น `manual` ทั้งหมด** — เจ้าหน้าที่ไล่ยืนยันจนครบแล้ว และทั้ง 22 ชุมชนมีจุดเก็บอย่างน้อย 1 จุด · ปุ่ม "รถขยะใกล้ฉัน" จึงครอบคลุมทุกคนที่อยู่ในเขต polygon
...
## หน้าประชาชนแบบไทม์ไลน์ (ตั้งแต่ M9)

หน้า `/garbage` เรียงจากบนลงล่าง: `GarbageHero` → `GarbageSearchPanel` → `DayOffNotice` → `RouteTimeline` → `PrepChecklist` → `CoverageNote`

- **ยิง API 3 เส้น:** `/week` (ตารางทั้งสัปดาห์ → หยิบวันนี้มาทำไทม์ไลน์ + หาวันที่ยังไม่มีตาราง) · `/settings` (เบอร์ติดต่อ) · `/live` ทุก 60 วิ (จำนวนรถที่วิ่ง + `nowMin` ฝั่งเซิร์ฟเวอร์) · **นาฬิกาเชื่อเซิร์ฟเวอร์ก่อน** ค่อย fallback มาที่เครื่องผู้ใช้เมื่อยิงไม่ผ่าน
- **แตะผลค้นหาเพื่อติดตามจุดได้** — ใช้ `TrackedStop` ตัวเดียวกับไทม์ไลน์ · กติกา toggle (แตะซ้ำ = เลิกติดตาม) อยู่ที่ `toggleTracked` ใน `pages/garbage.tsx` **ที่เดียว** ทั้งไทม์ไลน์และผลค้นหาเรียกตัวนี้ · `SearchHit` จึงต้องมี `seq` (คีย์คู่กับ `routeCode`) และ `truckColor` — `search.ts` อ่าน `garbage_trucks` แบบ **projection เฉพาะ `number`/`color`** เพราะเอกสารนั้นมี `driverName` ที่ห้ามหลุด API สาธารณะ · `weekday` ที่บันทึกคือวันของ hit ไม่ใช่วันนี้ (ติดตามจากวันไหนก็ได้) · แถวที่ไม่มี `seq`/`truckColor` (ผลจากแคช `s-maxage=300` รุ่นก่อน) ต้อง**กดไม่ได้** ไม่งั้นเขียน localStorage เป็นค่าที่ `parseTrackedStop` อ่านกลับไม่ได้ แล้วจุดหายเงียบ ๆ
- **ผลค้นหาแบ่งเป็นชิป 7 วัน** (`lib/garbage/searchDays.ts`) — ค้นชุมชนหนึ่งได้เฉลี่ย 27 แถว **สูงสุด 90 แถว** (ตาคลีใหญ่ 19 จุด × 7 วัน) ไล่ทุกวันพร้อมกันคือล้นจอมือถือ · ชิปเรียง **จันทร์→อาทิตย์** (ไม่ใช่ 0..6 ดิบ) วันที่รถไม่เข้าเป็นชิปปิด · จุดใต้ตัวอักษร = วันนี้ · เปิดที่วันของ **"รอบเก็บถัดไป"** เสมอโดยเรียก `findNextPickup` ตัวเดียวกับข้อความสรุปด้านบน — วันที่เลือกจึงตรงกับที่ข้อความนั้นพูดถึง ไม่มีกฎวันคนละชุด · ได้ผลชุดใหม่เมื่อไรต้องรีเซ็ตวันที่เลือก ไม่งั้นค้างที่วันที่ผลใหม่ไม่มีเก็บแล้วดูเหมือนไม่มีข้อมูล
- **ปุ่ม "รถขยะใกล้ฉัน"** ในหัว `GarbageSearchPanel` (`components/garbage/useLocateCommunity.ts`) — ขอพิกัด → `/api/citizen/locate-community` (polygon ชุมชน, **ใช้ร่วมกับปุ่ม "ชุมชนใกล้ฉัน" ของฟอร์มร้องเรียน**) → ได้ชื่อชุมชน → **เติมลงช่องค้นหา** ให้ flow ค้นหาเดิมทำงานต่อ · เติมลงช่องแทนการค้นเงียบ ๆ เพราะผู้ใช้ต้องเห็นว่าระบบเดาชุมชนไหนให้ และแก้เองได้ทันทีถ้าผิด · นอก polygon / ปฏิเสธสิทธิ์ = บอกให้พิมพ์เอง ไม่ใช่ error กลางหน้า · **อยู่ในชุมชนแต่ไม่มีจุดเก็บ ต้องขึ้นข้อความคนละแบบกับ "ไม่พบคำค้น"** ไม่งั้นคนอ่านว่า "ไม่พบ &lt;ชื่อชุมชนตัวเอง&gt;" แล้วนึกว่าพิมพ์ผิด
- **`buildRuns()` (`lib/garbage/timeline.ts`)** แปลงตารางของวันเป็นไทม์ไลน์รายสาย — เอาเฉพาะจุดที่ `served` ตัดงานวันหยุดและสายที่ไม่มีจุดเก็บ **คงลำดับที่ API ส่งมา** (เรียงตามเวลาออกวิ่ง ดูหัวข้อลำดับการแสดงผล)
- **`currentStopIndex()`** = จุดท้ายสุดในลิสต์ที่เวลาถึงแล้ว ไล่ตามลำดับลิสต์ ไม่ใช่หาค่ามากสุด เพราะลำดับจุดไม่ใช่ลำดับวิ่ง · จุดที่ไม่มีเวลาไม่นับ ไม่งั้นรถ 13 จะดูเหมือนอยู่ทุกจุดทั้งวัน
- **ต้องเช็ก `runStatus()` ก่อนใช้ `currentStopIndex()` เสมอ** — ค่านั้นมีความหมายเฉพาะตอน `running` ถ้าใช้ดื้อ ๆ **จุดสุดท้ายจะค้างเป็น "รถกำลังอยู่จุดนี้" ไปทั้งเย็นทั้งคืน** (เจอจริงตอนบ่ายสี่โมง โซน 1 ยังบอกว่ารถอยู่จุด 12.00 น.) · เลิกงานแล้วต้องขึ้น "วันนี้รถเก็บครบทุกจุดแล้ว" ทุกจุดขีดฆ่า ไม่มีการ์ดตำแหน่งรถ
- **รูปรถต้องอยู่บนไทม์ไลน์เสมอ** — แบบวาดรถไว้ในการ์ด "รถกำลังอยู่จุดนี้" ที่เดียว พอไม่มีการ์ดนั้น (เลิกงาน / ยังไม่ออก / ไม่มีเวลา) รถหายจากไทม์ไลน์ทั้งใบ ทั้งที่หัวเว็บกับการ์ดหน้าแรกยังมีรถ จึงย้ายรถไปจอดที่แถบสถานะด้วย `bob={false}` (bob คือท่า "จอดเก็บขยะอยู่" ซึ่งไม่จริงในสถานะพวกนี้)
- **สายที่เลิกงานแล้วยึด `lastTimedStopIndex()` ไม่ใช่จุดท้ายลิสต์** — โซน 3 วันศุกร์จุดท้ายลิสต์เก็บ 6.00 น. แต่รถเลิกงานจริง 11.15 น. ที่จุดกลางลิสต์ · และขอช่วงแบบ `{ behind: 5, ahead: 0 }` เพราะเลิกงานแล้วไม่มี "จุดถัดไป" ถ้าโชว์ต่อจะได้แถวที่รถแวะไปตั้งแต่เช้ามาต่อท้าย อ่านแล้วงง
...
แผนที่เส้นทาง (M8 ผูก `RouteStop.roadId` ให้แล้ว 81/172 จุด ที่เหลือยังไม่มีถนนตรงกัน) · export โปสเตอร์ · แจ้งเตือน LINE ก่อนรถถึง · จัดการทะเบียนรถจาก UI

- **เลือกชุมชนให้ 4 จุดของรถยกภาชนะรองรับ** (โซน 1–7 ครบแล้ว) — เป็นชื่อสถานที่ไม่ใช่ถนน สคริปต์เติมให้ไม่ได้ · 3 ใน 4 ลอกจากสายอื่นที่เก็บจุดชื่อเดียวกันได้ (รพ.ตาคลี→ตาคลีใหญ่ · ตลาดศรีเทพ→ศรีเทพ · ตลาดดวงนิมิต→ตาคลีพัฒนา)
- **จุดชื่อเดียวกันที่ชุมชนขัดกันข้ามโซน** — โรงเรียนประดับวิทย์ (โซน 1 = รจนา · โซน 5 = สว่างวงษ์) · ถนนพระสังข์ (โซน 3 = สังข์ทอง · โซน 7 = ศรีเทพ) · ถนนจันทร์เทวี (โซน 2 = ยศวิมล · โซน 6 = จันทร์เทวี) · centroid ของถนนตัดสินแทนไม่ได้เพราะถนนเส้นเดียวพาดหลายชุมชน
- **`/week` ส่งกลับ ~210KB** เพราะรวมจุดของทุกสายทั้ง 7 วัน — หน้าประชาชนใช้แค่วันนี้ (`/schedule` = 35KB) แต่ยังต้องใช้ `/week` หาวันที่ไม่มีตาราง ถ้าจะลดขนาดต้องมี endpoint บอก "วันไหนมีตาราง" แยก
- **นับถอยหลังบนการ์ดหน้าแรกข้ามวันไม่ได้** เพราะการ์ดไม่รู้ตารางของวันนี้ (ยิงแค่ `/live`) — แก้ได้ด้วยการให้ `/live` ส่ง `atMin` ของทุกจุดมาด้วย หรือให้การ์ดยิง `/schedule`
