...
- **ลำดับการแสดงผลสองฝั่งไม่เหมือนกันโดยตั้งใจ** — หน้าแอดมินเรียง **โซน 1 → โซน 7 → รถยกภาชนะรองรับ** ให้ตรงกับตารางกระดาษ (ตารางรายวันใน `WeekScheduleView` เรียงตาม `truckNumber` ไม่ใช่รหัสสาย เพราะแถววันหยุด `routeCode` เป็น `null` ถ้าเรียงด้วยรหัสสายจะหลุดไปกองท้ายตาราง · `GET /api/garbage/routes` เรียงด้วย `zoneOrder()` ในเครื่อง **ห้ามกลับไปใช้ `.sort({ code: 1 })`** เพราะรหัสเป็นสตริง จะได้ R1, R13, R2, …) ส่วน **หน้าประชาชนเรียงตามเวลาเริ่มวิ่ง** ใน `buildDaySchedule` เพราะคำถามของประชาชนคือ "รถคันไหนมาก่อน"
