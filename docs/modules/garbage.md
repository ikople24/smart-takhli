...
- **ข้อมูลใน DB ยังมีแค่จันทร์กับอังคาร** (ชุด seed เดิม) ทั้งสองหน้าต้องบอกตรง ๆ ว่ารอข้อมูล — ตารางฉบับจริงครบ 7 วันอยู่ใน `data/garbage/schedule-2569.json` แล้ว รอการนำเข้า
- **ตารางฉบับจริงขัดกฎของโมดูลเอง 2 ข้อ** ต้องตัดสินใจก่อนนำเข้า: (1) มี 10 งานที่เวลาไม่เรียงตาม `seq` (เช่น รถ 1 อาทิตย์ จุด 11 เวลา 5.00 มาหลังจุด 7 เวลา 5.10) ซึ่งชน refine "เวลาใน stopTimes ต้องไม่ย้อนกลับ" (2) มี 3 วันที่รถคันเดียวมีงาน `normal` และ `substitute` เวลาคาบเกี่ยวกัน (รถวิ่งสลับจุดของตัวเองกับจุดที่เก็บแทน) ซึ่งชนกฎห้ามเวลาทับของ `lib/garbage/overlap.ts` — งานเหล่านี้จะนำเข้าได้ แต่**เจ้าหน้าที่จะกดบันทึกจากหน้าแอดมินไม่ผ่าน** (400)
...
- `data/garbage/schedule-2569.json` — ตารางฉบับจริงจากกองสาธารณสุข (8 คัน 7 วัน) นำเข้าด้วย `scripts/import-garbage-schedule.mjs` · สคริปต์เป็น **re-baseline** (ลบ `garbage_assignments` ทิ้งแล้วเขียนใหม่) ต้องสั่ง `--yes` ชัดเจน และจะไม่ทับงานที่เคยแก้จากหน้าแอดมินจนกว่าจะใส่ `--force`
