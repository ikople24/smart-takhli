...
- **`PUT /api/garbage/routes/[code]` ต้องส่ง `updatedAt` ที่ได้จาก `GET /api/garbage/routes` กลับไปด้วย** (optimistic lock) — ฟอร์มที่ค้างจะได้ 409 ให้ปิดแล้วเปิดใหม่ ไม่ใช่เขียนทับเงียบ ๆ จนเวลาไปติดผิดจุด · ตอบกลับมี `warnings[]` ของงานที่เวลาเรียงย้อนหลังการสลับจุด หน้าแอดมินต้องแสดงแบบไม่หายเอง
...
- ไม่มี optimistic locking ของ**งานมอบหมาย**: สองคนแก้งานเดียวกันพร้อมกัน คนบันทึกทีหลังชนะ (ยอมรับได้ที่ขนาดทีมนี้) — มีเฉพาะฝั่งสาย
