...
- **ต้องเช็ก `runStatus()` ก่อนใช้ `currentStopIndex()` เสมอ** — ค่านั้นมีความหมายเฉพาะตอน `running` ถ้าใช้ดื้อ ๆ **จุดสุดท้ายจะค้างเป็น "รถกำลังอยู่จุดนี้" ไปทั้งเย็นทั้งคืน** (เจอจริงตอนบ่ายสี่โมง โซน 1 ยังบอกว่ารถอยู่จุด 12.00 น.) · เลิกงานแล้วต้องขึ้น "วันนี้รถเก็บครบทุกจุดแล้ว" ทุกจุดขีดฆ่า ไม่มีการ์ดตำแหน่งรถ ไม่มีรูปรถ
