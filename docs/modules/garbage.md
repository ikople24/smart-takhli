...
- `data/garbage/schedule-2569.json` — ตารางฉบับจริงจากกองสาธารณสุข (8 คัน 7 วัน) นำเข้าด้วย `scripts/import-garbage-schedule.mjs` → **สาย 8 · งาน 56 · รถ 8** (งาน = 7 คัน × 7 วัน + รถ 13 อีก 7 วัน · หนึ่งรถหนึ่งวันหนึ่งงานเสมอ)
  ...
  - **guard ก่อนเขียน:** ถ้าพบงานที่ `updatedAt` ห่างจาก `createdAt` เกิน 1 วินาที จะยกเลิกจนกว่าจะใส่ `--force` · เป็น heuristic ที่ **false positive ได้** ถ้ามีอะไรไป bulk-set `updatedAt` ทั้ง collection (ไม่ใช่การแก้จากหน้าแอดมินจริง) — ตรวจยืนยันด้วยว่ามี audit log `garbage_assignment_updated` จริงไหม และ `updatedAt` ของแต่ละงานต่างกันหรือเหมือนกันเป๊ะ ก่อนตัดสินใจใส่ `--force`
