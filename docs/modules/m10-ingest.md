...
### ข้อมูลติดต่อของเจ้าของ (ตรวจแล้ว 2026-09-21)
- แผ่นงานมีช่อง **โทรศัพท์** (`OWN_TEL`) และที่อยู่ครบอยู่แล้วผ่าน `OWNER_FIELD_COLS`
- แต่**ไฟล์กรมที่ดินไม่ส่งเบอร์โทรมาเลย** — งวด 2569-01 + 2569-02 รวม 176 รายการ
  `OWN_TEL` / `LESSOR_TEL` / `RENTER_TEL` ว่าง **0/176 ทุกช่อง** (และ `OWN_VILLAGE` ว่างทั้งหมด
  ส่วน `OWN_HSE_NO` มีครบ 176 · `OWN_ROAD` 78 · `OWN_SOI` 53 · `OWN_MOO` 48)
- แหล่งเดียวในฐานที่มีทั้งเบอร์และเลขบัตรคือ `elderly_people` (โรงเรียนผู้สูงอายุ) แต่
  **จับคู่กับเจ้าของที่ดินได้แค่ 1 จาก 96 คน (1%)** และเป็นข้อมูลที่เก็บมาเพื่อวัตถุประสงค์อื่น
  → **ไม่ทำการจับคู่ข้ามโมดูล** (ขัดหลักการใช้ข้อมูลตามวัตถุประสงค์ PDPA และแทบไม่ได้ประโยชน์)
- ช่องพร้อมรับค่าแล้ว ถ้าวันหนึ่งกรมที่ดินส่งเบอร์มาจะขึ้นบนแผ่นงานเองทันที (มีเทสต์ล็อกไว้)
- สแกนแล้วทั้ง 176 รายการ × 63 คอลัมน์ **ไม่มีค่าใดที่หน้าตาเป็นเบอร์โทรไทย** (`^0\d{8,9}$`) เลย
  คอลัมน์ที่ว่าง 100% ได้แก่ `OWN_TEL` `OWN_VILLAGE` `MOO_NO` `MERGE_PARCEL` และทั้งชุด
  `LESSOR_*` (15) + `RENTER_*` (15) — กรมที่ดินส่งโครงคอลัมน์มาครบแต่ไม่กรอก
- ช่องโทรศัพท์ที่ว่างจึงพิมพ์เป็น **เส้นประให้เจ้าหน้าที่จดเอง** (ไม่ใช่ขีด "—")
  คุมด้วย `WRITE_IN_WHEN_EMPTY` ใน `components/m10/print/WorkSheet.jsx`

## กฎจัดกลุ่มนิติกรรม + กู้รายการที่ถูกกัก (2026-09-21)
- **ปัญหา:** dictionary จับชื่อแบบตรงตัว แต่กรมที่ดินเขียนจำนวนโฉนดที่รวมเป็นคำไทยที่เปลี่ยนทุกเดือน
  ("รวมสองโฉนด" → "รวมสามโฉนด" → "รวมสิบหกโฉนด") งวด 03–05 จึงถูกกัก 122 แถวจาก 27 ชื่อ
- **กฎใน `classifyStatus()`** (ทำงานต่อเมื่อไม่เจอชื่อตรงตัวใน `STATUS_MAP` — ของเดิมมาก่อนเสมอ):
  - **กฎ A** ลงท้าย `/รวม\S*โฉนด$/` → `MERGE` เสมอ (ตรงกับทุกชื่อที่ใส่มือไว้เดิม) ครอบ 95 จาก 122 แถว
  - **กฎ B** ตัดวงเล็บต่อท้าย `/\s*\([^)]*\)$/` แล้วเทียบ dict ซ้ำ เช่น "โอนมรดก (ระหว่างจำนอง)"
  - ไม่เข้ากฎ → ยัง `null` → quarantine เหมือนเดิม **ห้ามเดา**
- **`npm run m10:reprocess-rejects`** (ใส่ `--yes` เพื่อเขียนจริง) — รันรายการที่ถูกกักผ่าน dictionary
  ล่าสุดซ้ำ **โดยไม่ต้องอัปโหลดไฟล์ใหม่และไม่ต้องล้างฐาน** เพราะ `m10_rejects` เก็บ `rawRow` ไว้ครบ
  · เรียงตาม `_id` เพื่อให้เจ้าของลำดับ 1 เข้าก่อน คนถัดไปจึงไปอยู่ `coOwnerRows` ถูกตัว
  · จบแล้วปรับ `batch.counts.rejects` ให้ตรงของจริง
  · **ใช้ทุกครั้งหลังเพิ่มนิติกรรมใหม่ใน `changeType.ts`**
- ⚠️ reject ของ geometry (`source: "geometry"`) **กู้ด้วยสคริปต์นี้ไม่ได้** เพราะเก็บแค่ `recordKey`
  ไม่ได้เก็บรูปแปลง — ต้องนำเข้าไฟล์เดิมซ้ำถึงจะได้รูปแปลงกลับมา (ยังเหลือ 60 แถว)
- ผลรันจริง 2026-09-21: กู้ 116 แถวเป็นรายการใหม่ + 6 แถวไปรวมเป็นเจ้าของร่วม → รวม 595 → 711 รายการ

### กฎกลุ่มรอบสอง — "ลำดับที่/ครั้งที่/เฉพาะส่วน" (2026-09-22)
งวด 06–08 โผล่ตระกูลตัวเลขไทยชุดใหม่อีก (จำนอง**ลำดับที่สาม/สี่** · ขยายเวลาไถ่**ครั้งที่หนึ่ง/สอง**)
พิสูจน์ว่าการไล่ใส่ชื่อทีละอันตามไม่ทันจริง จึงทำเป็นกฎเพิ่ม:
- `TRAILING_NOISE` = คำขยายท้ายที่ **ไม่เปลี่ยนชนิดนิติกรรม** → ตัดทีละชั้นแล้วเทียบ dict ซ้ำ
  (วงเล็บท้าย · `ลำดับที่X` · `ครั้งที่X`) ทำงานซ้อนกันได้ เช่น
  "ขยายกำหนดเวลาไถ่จากขายฝากครั้งที่สอง (กำหนดสามเดือน )" → ตัดวงเล็บ → ตัดครั้งที่ → ชื่อฐานใน dict
- `PARTIAL_SUFFIX` = ลงท้าย `เฉพาะส่วน` → หาชนิดของนิติกรรมฐาน แล้ว**แปลง TRANSFER เป็น
  TRANSFER_PARTIAL** ส่วนชนิดอื่นคงเดิม (จึงได้ "ขายเฉพาะส่วน" = โอนเฉพาะส่วน แต่
  "ไถ่ถอนจากจำนองเฉพาะส่วน" = ภาระผูกพัน และ "ผู้จัดการมรดกเฉพาะส่วน" = แก้ชื่อเจ้าของ)
- **กติกาคุมกฎ:** กฎทุกข้อต้องให้ผลตรงกับรายการที่ใส่มือไว้แล้วทุกตัว มีเทสต์เทียบตรง ๆ เช่น
  `classifyStatus("จำนองลำดับที่สาม")` ต้องเท่ากับ `classifyStatus("จำนองลำดับที่สอง")`
  และสิ่งที่ไม่รู้จักจริงต้องยังคืน null ("เฉพาะส่วน" ลอย ๆ · "ลำดับที่สอง" ลอย ๆ → null)
- ผลรันจริง 2026-09-22: กู้อีก 12 แถว + 1 แถวเป็นเจ้าของร่วม → **1,182 รายการ 8 งวด (ม.ค.–ส.ค. 2569)**
  เหลือถูกกัก 66 แถวซึ่งเป็น `geometry_unmatched` ล้วน ไม่มี `unknown_status` เหลือแล้ว
