...
- **จุดบนแผนที่ 3 ประเภท** (`FloodGauge.kind`: `gauge` วัดน้ำ · `water` แจกน้ำดื่ม · `donation` รับบริจาค — collection เดิม `flood_gauges`, เอกสารเก่าไม่มี kind = gauge): **จุดแจกน้ำ/รับบริจาคปักได้เฉพาะเจ้าหน้าที่** (กันจุดรับบริจาคปลอม) · **ประชาชนปักจุดวัดน้ำ + ส่งรูปอัปเดตได้จากหน้า `/flood`** (เจ้าของขอ 2026-09-26 "ช่วยกันปัก") ผ่าน `pages/api/flood-relief/public/gauges/*` ไม่ต้องล็อกอิน — บังคับแนบรูป · รับเฉพาะตอนศูนย์ฯ เปิด · 3 จุด + 10 รูป/ชม./IP (`lib/flood-relief/publicPoints.ts`) · ขึ้นทันทีพร้อมป้าย "ภาพจากประชาชน" (`source`/`lastSource`) · เจ้าหน้าที่ซ่อนได้ ลบ superadmin · `clientIp` ห้ามส่งออก API
