...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ปุ่มติดตามผ่าน LINE — oaMessage deep link เปิดแชทบอทพร้อมข้อความ "สถานะ <เลขเรื่อง>"
      // เตรียมไว้ให้ กดส่งทีเดียวจบ (ถ้ายังไม่ได้เพิ่มเพื่อน LINE พาไปเพิ่มก่อนอัตโนมัติ)
      // ถ้า parse basicId จาก NEXT_PUBLIC_LINE_OA_URL ไม่ได้ fallback เป็นลิงก์เพิ่มเพื่อนปกติ
      let lineHtml = '';
      if (lineOaUrl) {
        ...
        const followUrl = oaBasicId
          ...
          : lineOaUrl;
        lineHtml = `
          <p style="margin:12px 0 4px;font-size:14px;color:#555">รับแจ้งเตือนความคืบหน้าอัตโนมัติทาง LINE<br/>กดปุ่มด้านล่าง แล้วกดส่งข้อความที่เตรียมไว้ให้</p>
