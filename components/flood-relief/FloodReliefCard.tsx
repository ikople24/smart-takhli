...
export default function FloodReliefCard() {
  ...
  return (
    <section
      ...
      className="relative mx-4 mt-3 overflow-hidden rounded-[22px] bg-tk-flood p-4 shadow-tk-flood-card"
    >
      ...
      >
        <path d="M0 36 C 45 22 90 50 135 36 S 225 22 270 36 S 330 46 358 34 V104 H0 Z" fill="#2250B3" />
        <path d="M0 62 C 50 50 100 74 150 62 S 250 50 300 62 S 340 68 358 60 V104 H0 Z" fill="#2A5DC6" />
      </svg>
      ...
      <div className="relative flex flex-col">
        <div className="flex items-center justify-between gap-2">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/15 py-[5px] pl-2.5 pr-[11px] text-[11.5px] font-semibold text-white">
            <span className="relative inline-block h-2 w-2">
              ...
              <span className="absolute inset-0 rounded-full" style={{ background: situation.dot }} />
            </span>
        ...
        {summary.announcement && (
          <p className="mt-2 rounded-xl bg-white/12 px-3 py-2 text-[12px] leading-normal text-white">{summary.announcement}</p>
        )}
        ...
        <div className="mt-3.5 grid grid-cols-4 gap-1">
          {REQUEST_TYPES.map((t) => (
            ...
            >
              <span className="flood-disc flex h-[62px] w-[62px] items-center justify-center rounded-full bg-white text-tk-flood shadow-[0_4px_10px_rgba(8,24,70,0.25)]">
        ...
        <Link
          href="/flood/request"
          className="mt-4 flex h-[54px] items-center justify-center gap-2.5 rounded-2xl bg-white text-[16.5px] font-bold text-tk-flood shadow-[0_6px_14px_rgba(8,24,70,0.28)] transition hover:-translate-y-px motion-reduce:transition-none motion-reduce:hover:translate-y-0"
        >
