...
export default function FloodReliefCard() {
  ...
  return (
    // กรอบไฟไซเรน (.flood-siren ใน globals.css) ห่อการ์ดไว้ — ขอบ 3px ที่หมุนสีแดง/น้ำเงิน
    <div className="flood-siren mx-4 mt-3">
      <section aria-labelledby="flood-title" className="relative overflow-hidden rounded-[22px] bg-tk-emergency p-4">
        ...
        >
          <path
            d="M0 36 C 45 22 90 50 135 36 S 225 22 270 36 S 330 46 358 34 V104 H0 Z"
            fill="var(--color-tk-emergency-wave)"
          />
          <path
            d="M0 62 C 50 50 100 74 150 62 S 250 50 300 62 S 340 68 358 60 V104 H0 Z"
            fill="var(--color-tk-emergency-wave-2)"
          />
        </svg>
        ...
        <div className="relative flex flex-col">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <span className="relative inline-block h-2 w-2">
                ...
                <span
                  className="absolute inset-0 rounded-full ring-[1.5px] ring-white"
                  style={{ background: situation.dot }}
                />
              </span>
          ...
          {summary.announcement && (
            <p className="mt-2 rounded-xl bg-black/15 px-3 py-2 text-[12px] leading-normal text-white">
              {summary.announcement}
            </p>
          )}
          ...
          <div className="mt-2.5 flex items-center justify-center gap-4">
            <Link
              href="/flood/status"
              className="inline-flex min-h-9 items-center gap-1.5 text-[12.5px] font-semibold text-white"
            >
