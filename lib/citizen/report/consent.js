...
/** ความยาวสูงสุดของ acceptedAt ก่อนส่งเข้า Date — ISO 8601 ยาวประมาณ 24 ตัวอักษร ยาวเกินนี้มากไม่ใช่เวลาแล้ว (กัน string ยักษ์กิน CPU ตอน parse) */
export const ACCEPTED_AT_MAX_LENGTH = 40;
...
export function validateConsentLog(body, now = new Date()) {
  try {
    ...
    // ยาวเกิน ACCEPTED_AT_MAX_LENGTH ถือว่าใช้ไม่ได้ทันที ไม่ส่งเข้า new Date() เลย (กัน CPU cost จาก string ยักษ์)
    const acceptedAtUsable = typeof acceptedAt === "string" && acceptedAt.length <= ACCEPTED_AT_MAX_LENGTH;
    let when = acceptedAtUsable ? new Date(acceptedAt) : new Date(NaN);
...
/**
 * ข้อมูลยินยอมที่อนุญาตให้แนบไปกับ payload ตอนส่งเรื่องร้องเรียน — จุดเดียวที่ตัดสินว่า "เอาไปด้วยได้ไหม"
 *
 * ต้องเข้มกว่า parseStoredConsent เพราะปลายทางคือฟิลด์ Date ใน Mongoose schema: acceptedAt ที่ parse
 * ไม่ออก (หรือยาวผิดปกติ) จะทำให้ SubmittedReport.create() throw CastError ซึ่งเซฟ "ทั้งเรื่องร้องเรียน"
 * ไม่ผ่านเลย ไม่ใช่แค่ส่วน consent — จึงต้องทิ้งหลักฐานตรงนี้แทนที่จะปล่อยของเสียไปให้ DB
 * ยอมรับจริงยังพิสูจน์ได้จาก consent log ฝั่งเซิร์ฟเวอร์ (POST /api/complaints/consent-log) อยู่แล้ว
 * ซึ่งเป็นแหล่งความจริงหลัก การไม่แนบใน payload นี้จึงไม่ใช่การเสียหลักฐาน
 *
 * @param {unknown} raw
 * @returns {{ version: string, acceptedAt: string } | null}
 */
export function consentForPayload(raw) {
  if (!raw || typeof raw !== "object") return null;
  const { version, acceptedAt } = raw;
  if (typeof version !== "string" || version === "") return null;
  if (typeof acceptedAt !== "string" || acceptedAt === "" || acceptedAt.length > ACCEPTED_AT_MAX_LENGTH) {
    return null;
  }
