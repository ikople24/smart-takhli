...
describe("handlingSpeed — ป้ายความเร็ว 5 ระดับตามเกณฑ์เดิม", () => {
  ...
  it("วันที่สลับกัน (completed ก่อน assigned) = ข้อมูลเพี้ยน → null ไม่เดาค่าสัมบูรณ์", () => {
    // ข้อมูลเก่าบางเรื่อง completedAt เป็น date-only เที่ยงคืน UTC จน "เสร็จก่อนรับงาน"
    expect(handlingSpeed(at("2026-08-02T08:00:00Z"), at("2026-08-01T20:00:00Z"))).toBeNull();
  });
});
...
describe("handlingDuration — เวลาที่ใช้แบบอ่านง่าย", () => {
  ...
  it("ข้อมูลไม่ครบ/วันที่สลับกัน → null", () => {
    expect(handlingDuration(null, null)).toBeNull();
    expect(handlingDuration(at("2026-08-02T08:00:00Z"), at("2026-08-01T20:00:00Z"))).toBeNull();
  });
});
