...
// ยกเว้นจุดเดียว: completedAt ก่อน assignedAt (ข้อมูลเก่าเพี้ยน) → null แทน Math.abs (2026-09-11)
...
export function handlingSpeed(assignedAt, completedAt) {
  ...
  const diff = new Date(completedAt) - new Date(assignedAt);
  // ติดลบ = ข้อมูลเพี้ยน (flow เก่าเก็บ completedAt แบบ date-only เที่ยงคืน UTC) — ไม่แสดงป้ายดีกว่าเดาเวลา
