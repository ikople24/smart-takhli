...
describe("buildComplaintPayload — หลักฐานการยอมรับข้อตกลง", () => {
  ...
  it("มี acceptedAt แต่ไม่มี version → ไม่แนบคีย์ consent", () => {
    const p = buildComplaintPayload(
      { ...state, consent: { acceptedAt: consent.acceptedAt } },
      problemOptions
    );
    expect(p.consent).toBeUndefined();
  });

  it("acceptedAt parse เป็นวันที่ไม่ได้ (junk) → ไม่แนบคีย์ consent (กัน CastError ตอนเซฟ SubmittedReport)", () => {
    const p = buildComplaintPayload(
      { ...state, consent: { ...consent, acceptedAt: "not-a-date" } },
      problemOptions
    );
    expect(p.consent).toBeUndefined();
  });

  it("ข้อมูลยินยอมถูกต้อง → ยังแนบ consent ตามปกติ (ล็อก happy path ไว้)", () => {
    const p = buildComplaintPayload({ ...state, consent }, problemOptions);
    expect(p.consent).toEqual(consent);
  });
});
