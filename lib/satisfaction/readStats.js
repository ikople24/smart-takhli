...
//          pages/api/tasks/my-kpi.ts (loadSatisfactionStatsForComplaints — KPI รายเจ้าหน้าที่)
 ...
 *   reports: Map<string, { phone?: string|null, lineUserId?: string|null }>
 * }>}
 */
export async function loadSatisfactionStats({ from } = {}) {
  await dbConnect();
  const filter = from ? { createdAt: { $gte: from } } : {};
  return hydrateRows(await Satisfaction.find(filter).select("complaintId rating source createdAt").lean());
}

/**
 * เหมือน loadSatisfactionStats แต่จำกัดเฉพาะเรื่องที่ระบุ — ใช้ทำ KPI ความพึงพอใจรายเจ้าหน้าที่
 * (my-kpi ส่ง complaintId ของงานที่เจ้าหน้าที่คนนั้นถือ) ผลลัพธ์ส่งเข้า computeFairStats เหมือนกัน
 * @param {Array<string | import("mongoose").Types.ObjectId>} complaintIds
 */
export async function loadSatisfactionStatsForComplaints(complaintIds) {
  await dbConnect();
  const ids = [...new Set((complaintIds ?? []).map(String))].filter((id) =>
    mongoose.Types.ObjectId.isValid(id)
  );
  if (!ids.length) return { ratings: [], reports: new Map() };
  return hydrateRows(
    await Satisfaction.find({ complaintId: { $in: ids } }).select("complaintId rating source createdAt").lean()
  );
}

// แถวคะแนน → { ratings, reports } (ข้อมูลผู้แจ้งเฉพาะที่ reporterKey ใช้)
