...
describe('reporterKey', () => {
  ...
  it('มีทั้งเบอร์และ lineUserId → เบอร์ชนะ · เบอร์เป็นขีดล้วน → ตกไป line', () => {
    expect(reporterKey({ phone: '0810000001', lineUserId: 'Uabc' }, 'c1')).toBe('tel:0810000001');
    expect(reporterKey({ phone: '-', lineUserId: 'Uabc' }, 'c1')).toBe('line:Uabc');
  });
});
...
describe('computeFairStats', () => {
  ...
  it('ปัด 4 ตำแหน่ง — ค่าที่ endpoint สาธารณะคืนไม่ยาวเกิน', () => {
    const s = computeFairStats([R('a', 1), R('b', 5), R('c', 5)]);
    expect(s.averageRating).toBe(3.6667);
    expect(s.rawAverage).toBe(3.6667);
    expect(s.bySource.public.average).toBe(3.6667);
  });

  it('คะแนนต่างช่องทางในเรื่องเดียวกัน: เฉลี่ยรวมต่อเรื่อง แต่ bySource แยกดิบ', () => {
    const s = computeFairStats([R('x', 5, 'public'), R('x', 1, 'line')], reports({ x: { phone: '0810000001' } }));
    expect(s.averageRating).toBeCloseTo(3, 3);
    expect(s.ratedComplaints).toBe(1);
    expect(s.bySource.public).toEqual({ count: 1, average: 5 });
    expect(s.bySource.line).toEqual({ count: 1, average: 1 });
  });

  it('แถวที่ rating ไม่ใช่ตัวเลขถูกข้าม ไม่ทำให้ค่าเฉลี่ยเป็น NaN', () => {
    const s = computeFairStats([R('a', undefined), R('b', 4), R('c', 'x')], reports({}));
    expect(s.totalRatings).toBe(1);
    expect(s.averageRating).toBe(4);
    expect(s.rawAverage).toBe(4);
    expect(s.reporters).toBe(1);
    expect(s.ratedComplaints).toBe(1);
  });

  it('reports ที่ไม่ใช่ Map (null/object) ไม่ทำให้ล้ม — ทุกเรื่องเป็นเสียงของตัวเอง', () => {
    expect(computeFairStats([R('a', 2), R('b', 4)], null).averageRating).toBe(3);
    expect(computeFairStats([R('a', 2), R('b', 4)], {}).reporters).toBe(2);
    expect(computeFairStats(null).totalRatings).toBe(0);
  });
});
