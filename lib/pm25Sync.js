...
export async function syncPm25Monthly() {
  ...
  try {
    ...
    // และอัปเดตเดือนปัจจุบันให้ทันข้อมูลล่าสุด) — ยกเว้นเดือนขอบหน้าต่างที่
    // ข้อมูลรอบนี้บางกว่าที่เก็บไว้แล้ว อย่าให้ทับของเดิมที่สมบูรณ์กว่า
    ...
    const toUpsert = months.filter((m) => {
      ...
      return existingCount === undefined || m.count >= existingCount;
    });
