...
describe("planBulkGrant", () => {
  ...
  it("revoke หน้าเดียวที่มี → ข้าม (ลิสต์จะว่าง เด้งกลับ default = อาจได้สิทธิ์เพิ่ม)", () => {
    const plan = planBulkGrant([doc("a", APP, ["/admin/garbage"])], ["a"], APP, {
      mode: "revoke",
      pagePath: "/admin/garbage",
    });
    expect(plan.applyIds).toEqual([]);
    expect(plan.skippedWouldEmpty).toEqual(["a"]);
  });

  it("revoke โดยยังเหลือหน้าอื่น → apply", () => {
    const plan = planBulkGrant([doc("a", APP, ["/admin/garbage", "/admin/dashboard"])], ["a"], APP, {
      mode: "revoke",
      pagePath: "/admin/garbage",
    });
    expect(plan.applyIds).toEqual(["a"]);
    expect(plan.skippedWouldEmpty).toEqual([]);
  });

  it("revoke หน้าที่ user ไม่มีอยู่แล้ว → apply ($pull เป็น no-op ปลอดภัย)", () => {
    const plan = planBulkGrant([doc("a", APP, ["/admin/dashboard"])], ["a"], APP, {
      mode: "revoke",
      pagePath: "/admin/garbage",
    });
    expect(plan.applyIds).toEqual(["a"]);
