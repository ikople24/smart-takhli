...
import { HEAD_POSITION_RE, isDepartmentHead, headsOf, taskPermissions, canClaim } from "../roles";
...
describe("canClaim — กติการับงานเอง (บังคับฝั่ง server ใช้ทั้ง task-pool และหน้าการร้องเรียน)", () => {
  const officer = user({ position: "นายช่างโยธา", department: "กองช่าง" });
  it("กองเดียวกัน / เรื่องยังไม่ระบุกอง / superadmin / โปรไฟล์ไม่ระบุกอง → รับได้", () => {
    expect(canClaim({ isSuperAdmin: false, user: officer, taskDepartment: "กองช่าง" })).toBe(true);
    expect(canClaim({ isSuperAdmin: false, user: officer, taskDepartment: null })).toBe(true);
    expect(canClaim({ isSuperAdmin: true, user: officer, taskDepartment: "กองคลัง" })).toBe(true);
    expect(canClaim({ isSuperAdmin: false, user: user({ department: "" }), taskDepartment: "กองคลัง" })).toBe(true);
  });
  it("เรื่องของกองอื่น → รับไม่ได้ (แม้เป็นหัวหน้ากองของกองตัวเอง)", () => {
    expect(canClaim({ isSuperAdmin: false, user: officer, taskDepartment: "กองคลัง" })).toBe(false);
    expect(canClaim({ isSuperAdmin: false, user: user({ position: "ผู้อำนวยการกองช่าง" }), taskDepartment: "กองคลัง" })).toBe(false);
  });
  it("ชื่อกองใน users.department สะกดแบบ alias ก็เทียบถูก", () => {
    expect(canClaim({ isSuperAdmin: false, user: user({ department: "ช่าง" }), taskDepartment: "กองช่าง" })).toBe(true);
  });
});
