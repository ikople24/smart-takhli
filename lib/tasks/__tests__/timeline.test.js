...
describe("closeChecklist — สรุปบังคับเสมอ · ภาพ ≥1 หรือติ๊กยืนยันปิดโดยไม่มีภาพ", () => {
  ...
  it("มีภาพ + สรุป → ok โดยไม่ต้องติ๊ก (พฤติกรรมเดิม · ติ๊กมาด้วยก็ไม่มีผล)", () => {
    expect(closeChecklist({ note: "เปลี่ยนโคมไฟแล้ว", images: ["a.jpg"] })).toEqual({ ok: true, errors: [] });
    expect(closeChecklist({ note: "เปลี่ยนโคมไฟแล้ว", images: ["a.jpg"], confirmNoImages: true })).toEqual({ ok: true, errors: [] });
  });
  ...
  it("ไม่มีภาพ+ไม่ติ๊ก+มีสรุป → error ภาพอย่างเดียว · ติ๊กแต่สรุปว่าง → error สรุปอย่างเดียว", () => {
    ...
    // กัน refactor เผลอเปลี่ยน !== true เป็น !confirmNoImages — string "true" ต้องไม่นับเป็นการยืนยัน
    expect(closeChecklist({ note: "ทำแล้ว", images: [], confirmNoImages: "true" }).ok).toBe(false);
  });
});
