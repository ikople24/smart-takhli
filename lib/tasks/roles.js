...
/**
 * รับงานเอง (claim): เรื่องต้องอยู่กองเดียวกับเจ้าหน้าที่ · เรื่องยังไม่ระบุกอง / โปรไฟล์ไม่ระบุกอง / superadmin → ได้
 * (หัวหน้ากองไม่มีสิทธิ์พิเศษข้ามกอง — ถ้าจะย้ายข้ามกองให้ใช้มอบหมาย/โอนตามสาย)
 * บังคับฝั่ง server ที่ POST /api/complaints/assignments/create — ใช้ทั้งกองงานรอรับและหน้าการร้องเรียน
 * @param {{ isSuperAdmin?: boolean, user?: object, taskDepartment?: string | null }} p
 */
export function canClaim({ isSuperAdmin = false, user, taskDepartment = null } = {}) {
  if (isSuperAdmin) return true;
  const ownDept = normalizeDepartment(user?.department);
  const target = taskDepartment ? (normalizeDepartment(taskDepartment) ?? taskDepartment) : null;
  return !target || !ownDept || ownDept === target;
}
